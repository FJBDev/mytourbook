...
public class PrefPageComputedValues extends PreferencePage implements IWorkbenchPreferencePage {
   ...
   public static final String  URL_DOUGLAS_PEUCKER_ALGORITHM     = "https://en.wikipedia.org/wiki/Ramer%E2%80%93Douglas%E2%80%93Peucker_algorithm"; //$NON-NLS-1$
...
//		container.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_MAGENTA));
      {
         ...
         _comboBreakMethod.addSelectionListener(SelectionListener.widgetSelectedAdapter(
               selectionEvent -> {
         ...
         GridLayoutFactory.fillDefaults().applyTo(btnContainer);
         {
            ...
            btnComputeValues.addSelectionListener(SelectionListener.widgetSelectedAdapter(
...
//		container.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_RED));
      {
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakMinAvgSpeedAS);
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakMinSliceSpeedAS);
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakMinSliceTimeAS);
...
//		container.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_RED));
      {
         ...
         GridDataFactory.fillDefaults().applyTo(_spinnerBreakMinAvgSpeed);
...
//		container.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_BLUE));
      {
         ...
         GridDataFactory.fillDefaults().applyTo(_spinnerBreakMinSliceSpeed);
   ...
   private Composite createUI_54_BreakByTimeDistance(final Composite parent) {
      ...
      GridLayoutFactory.fillDefaults().numColumns(3).applyTo(container);
      {
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakShortestTime);
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakMaxDistance);
...
//						.span(2, 1)
          ...
          */
         {
            ...
            GridDataFactory.fillDefaults().applyTo(_spinnerBreakSliceDiff);
   ...
   private Control createUI_60_PaceSpeed(final Composite parent) {

      final Composite container = new Composite(parent, SWT.NONE);
      GridDataFactory.fillDefaults().grab(true, false).applyTo(container);
      GridLayoutFactory.swtDefaults().numColumns(2).extendedMargins(5, 5, 10, 5).applyTo(container);
      {
         /*
         ...
         GridLayoutFactory.fillDefaults().numColumns(1).applyTo(innerContainer);
         {
            {
               // Recorded time
               _rdoDeviceTime_Recorded = new Button(innerContainer, SWT.RADIO);
            ...
            {
               // Moving time
               _rdoComputedTime_Moving = new Button(innerContainer, SWT.RADIO);
   ...
   private Control createUI_70_ElevationGain(final Composite parent) {
...
//		container.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_BLUE));
      {
         ...
         GridLayoutFactory.fillDefaults().numColumns(3).applyTo(dpContainer);
         {
            ...
            linkDP.addSelectionListener(SelectionListener.widgetSelectedAdapter(
   ...
   private Control createUI_80_CadenceZones(final Composite parent) {
      ...
      GridLayoutFactory.swtDefaults().numColumns(3).extendedMargins(0, 0, 7, 0).applyTo(container);
      {
         ...
         GridDataFactory.fillDefaults().applyTo(_spinnerCadenceDelimiter);
      ...
      buttonComputeTimes.addSelectionListener(SelectionListener.widgetSelectedAdapter(
   ...
   private void onComputeCadenceZonesTimeValues() {
      ...
      final IComputeNoDataserieValues computeTourValueConfig = new IComputeNoDataserieValues() {
         ...
         public String getResultText() {
            ...
            return UI.NEW_LINE + NLS.bind(
                  Messages.Compute_CadenceZonesTimes_ComputeForAllTours_Job_Result,
                  new Object[] {
                        UI.format_hh_mm_ss(total_Old_CadenceZone_SlowTime[0]),
                        UI.format_hh_mm_ss(total_New_CadenceZone_SlowTime[0]),
                        UI.format_hh_mm_ss(total_Old_CadenceZone_FastTime[0]),
                        UI.format_hh_mm_ss(total_New_CadenceZone_FastTime[0]), });
         }
   ...
   private void onComputeElevationGainValues() {
      ...
      final String dpToleranceWithUnit = _nf1.format(prefDPTolerance) + UI.SPACE1
      ...
      final IComputeTourValues computeTourValueConfig = new IComputeTourValues() {
         ...
         private String getElevationDifferenceString(final int elevationDifference) {
            ...
            if (elevationDifference > 0) {
               differenceResult.append(UI.SYMBOL_PLUS);
            }
   ...
   protected void performDefaults() {
      ...
      if (selectedTab == TAB_FOLDER_ELEVATION_GAIN) {

         /*
          * compute altitude
          */
         final float prefDPTolerance = _prefStore.getDefaultFloat(//
               ITourbookPreferences.COMPUTED_ALTITUDE_DP_TOLERANCE) * 10 / UI.UNIT_VALUE_ELEVATION;
...
// SET_FORMATTING_OFF
            ...
            /*
             * Break by avg+slice speed (AS)
             */
            final float prefMinAvgSpeedAS    = _prefStore.getDefaultFloat(ITourbookPreferences.BREAK_TIME_MIN_AVG_SPEED_AS);
            final float prefMinSliceSpeedAS  = _prefStore.getDefaultFloat(ITourbookPreferences.BREAK_TIME_MIN_SLICE_SPEED_AS);
            final int prefMinSliceTimeAS     = _prefStore.getDefaultInt(ITourbookPreferences.BREAK_TIME_MIN_SLICE_TIME_AS);

            _spinnerBreakMinAvgSpeedAS       .setSelection((int) (prefMinAvgSpeedAS * SPEED_DIGIT_VALUE / UI.UNIT_VALUE_DISTANCE));
            _spinnerBreakMinSliceSpeedAS     .setSelection((int) (prefMinSliceSpeedAS * SPEED_DIGIT_VALUE / UI.UNIT_VALUE_DISTANCE));
...
// SET_FORMATTING_ON
   ...
   private void restoreState() {
      ...
      _isUpdateUI = true;
      {
          ...
          * DP tolerance
          */
         final float prefDPTolerance = _prefStore.getFloat(ITourbookPreferences.COMPUTED_ALTITUDE_DP_TOLERANCE) * 10 / UI.UNIT_VALUE_ELEVATION;
         _spinnerDPTolerance.setSelection((int) prefDPTolerance);
 ...
 */
   ...
   private void saveState() {
      ...
      if (_initialUnitHashCode != UI.UNIT_HASH_CODE) {
         return;
      }
      ...
      _prefStore.setValue(
            ...
            _spinnerDPTolerance.getSelection() / 10.0f * UI.UNIT_VALUE_ELEVATION);
       ...
       */
      final float breakMinAvgSpeedAS = _spinnerBreakMinAvgSpeedAS.getSelection() / SPEED_DIGIT_VALUE * UI.UNIT_VALUE_DISTANCE;
      final float breakMinSliceSpeedAS = _spinnerBreakMinSliceSpeedAS.getSelection() / SPEED_DIGIT_VALUE * UI.UNIT_VALUE_DISTANCE;
       ...
       */
      final float breakMinSliceSpeed = _spinnerBreakMinSliceSpeed.getSelection() / SPEED_DIGIT_VALUE / UI.UNIT_VALUE_DISTANCE;
      _prefStore.setValue(ITourbookPreferences.BREAK_TIME_MIN_SLICE_SPEED, breakMinSliceSpeed);

      /*
       * Break by avg speed
       */
      final float breakMinAvgSpeed = _spinnerBreakMinAvgSpeed.getSelection() / SPEED_DIGIT_VALUE / UI.UNIT_VALUE_DISTANCE;
   ...
   private void updateUIShowSelectedBreakTimeMethod() {
      ...
      UI.updateScrolledContent(_tabFolder);
   }
