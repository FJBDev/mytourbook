...
public class ChartOptions_TrainingLoad implements IStatisticOptions {
   ...
   private Button                 _chkShowChronicTrainingLoad;
   private Button                 _chkShowAcuteTrainingLoad;
   private Button                 _chkShowPredictedPerformance;

   private TrainingStressPrefKeys _prefKeys;

   public ChartOptions_TrainingLoad(final TrainingStressPrefKeys prefKeys) {
   ...
   private void createUI_20_Values(final Composite parent) {
      ...
      final Group group = new Group(parent, SWT.NONE);
      group.setText("Values");//Messages.Pref_Statistic_Group_Training);
      GridDataFactory.fillDefaults()//
            .grab(true, false)
            .span(2, 1)
            .applyTo(group);
      ...
      {
         {
            ...
            _chkShowChronicTrainingLoad = new Button(group, SWT.CHECK);
            _chkShowChronicTrainingLoad.setText("CTL");//Messages.Pref_Statistic_Checkbox_Distance);
            // _chkShowChronicTrainingLoad.addSelectionListener(_defaultSelectionListener);
         }
         {
            /*
             * Show altitude
             */
            _chkShowAcuteTrainingLoad = new Button(group, SWT.CHECK);
            _chkShowAcuteTrainingLoad.setText("ATL");//Messages.Pref_Statistic_Checkbox_Altitude);
            //   _chkShowAcuteTrainingLoad.addSelectionListener(_defaultSelectionListener);
         }
      }
   }
