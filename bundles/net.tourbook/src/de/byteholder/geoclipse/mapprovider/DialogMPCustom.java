...
//
   ...
   private void createUI230UserAgent(final Composite parent) {
      ...
      GridLayoutFactory.fillDefaults().numColumns(2).applyTo(container);
      {
         ...
         // label: image format value
         _txtUserAgent = new Text(container, SWT.NONE);
         GridDataFactory.fillDefaults().grab(true, false).applyTo(_txtUserAgent);
         _txtUserAgent.setToolTipText(Messages.Dialog_CustomConfig_Text_ImageFormat_Tooltip);
         _txtUserAgent.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
         _txtUserAgent.setBackground(Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND));
